...
- **换号重试**：429/5xx/rate limit / 额度耗尽等可恢复错误会排除当前号。换号前对未冷却候选**并行刷新额度**（2.5s 超时，失败保留缓存），再选剩余额度最大者；单请求最多尝试 16 个账号。新会话只用缓存额度，不打 billing。全部失败时把最后一次上游错误返回给客户端，不会变成「重试深度超限」
